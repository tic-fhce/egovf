...
<div class="container mt-4">
    <div style="display: flex; align-items:center; gap: 12px; justify-content: space-between;">
        ...
        <CNavbarNav >
        <CDropdown >
          <CDropdownToggle class="dropdown-bandeja" :navLink="false">Todas las Solicitudes</CDropdownToggle>
          <CDropdownMenu>
...
const title = 'Todas las Solicitudes'
const loading = ref(true)
const error = ref(null)
const solicitudes = ref([])
const inventarioService = new InventarioService()
const listaAtenciones = computed(() => {
...
const cargarTodas = async () => {
  try {
    const response = await inventarioService.getSolicitudes()
...
const cargarAtendidas = async () => {
  try {
    const response = await inventarioService.getSolicitudesAtendidas()
    solicitudes.value = response.data
  } catch (err) {
    error.value = 'No se pudieron cargar las solicitudes.'
    console.error(err)
...
const cargarEnEspera = async () => {
  try {
    const response = await inventarioService.getSolicitudesEnEspera()
    solicitudes.value = response.data
  } catch (err) {
    error.value = 'No se pudieron cargar las solicitudes.'
    console.error(err)
