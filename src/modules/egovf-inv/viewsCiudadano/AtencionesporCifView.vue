<template>
    <nav v-if="sw == 'admin'" aria-label="breadcrumb">
...
<div class="container mt-4">
    ...
    <div v-for="(solicitud, index) in listaAtenciones" :key="index" :class="[solicitud.estado == 0 ? 'atencion-card' : 'solicitud-card']" class="mb-3 p-3 border-left">
      ...
      <div v-else>
      <CButton v-if="sw == 'admin'" color="danger" style="width: 100%;" variant="ghost" @click="atenderSolicitud(solicitud.idSolicitud, solicitud.idEquipo)">Atender</CButton>
      </div>
...
export default {
  ...
  async created() {
    ...
    const response = await this.inventarioService.getSolicitudesporCif(this.cif);
    this.solicitudes = response.data;
    this.listaAtenciones = this.solicitudes.map(solicitud => ({
      ...solicitud,
      estado: solicitud.estado || 0,
    }));
  },
  async cargarAtendidas() {
    this.title = 'Solicitudes Atendidas';
    this.listaAtenciones = this.solicitudes
  ...
  async cargarEnEspera() {
    ...
    this.listaAtenciones = this.solicitudes
      .filter(solicitud => solicitud.estado === 0)
      .map(solicitud => ({
        ...solicitud,
        estado: solicitud.estado || 0,
      }));
