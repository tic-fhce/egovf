              ...
              <tbody>
                <tr v-for="equipo in listaEquipo" :key="equipo.id">
                  ...
                  <td>
                    ...
                    <CBadge v-if="equipo.estado == 1" color="success"
                      >Activo</CBadge
                    >
                  ...
                  <td>
                    <CButton
                      class="font"
                      color="success"
                      @click="equipoSelect(equipo.id)"
                      size="sm"
                      ><CIcon icon="cil-devices" class="me-2" />Equipo</CButton
                    >
                  </td>
                </tr>
              </tbody>
        ...
        <div class="mb-3 row">
          ...
          <div class="col-8">
            <input
              type="text"
              class="form-control"
              v-model="equipo.detalle"
              placeholder="Nombre del lugar donde se encuentra el Equipo Biometrico"
              required="true"
            />
          </div>
        </div>
        ...
        <div class="mb-3 row">
          ...
          <div class="col-8">
            <input
              ...
              placeholder="Iniciales del lugar donde se encuentra el Equipo Biometrico"
              required="true"
            />
          </div>
        </div>
        ...
        <div class="mb-3 row">
          ...
          <div class="col-8">
            <input
              ...
              required="true"
              placeholder="Codigo de inventario del equipo"
            />
          </div>
        </div>
      </CModalBody>
      <CModalFooter>
        <CButton @click="clickModalEquipo(false)" color="danger" class="font"
  ...
  <CModal :visible="modalEquipoUpdate" @close="clickModalEquipoUpdate(false)">
    <form @submit.prevent="updateEquipo()">
      <CModalHeader
        class="headercolor"
        dismiss
        @close="clickModalEquipoUpdate(false)"
      >
        <CModalTitle>
      ...
      <CModalFooter>
        <CButton
          @click="clickModalEquipoUpdate(false)"
          color="danger"
          class="font"
          ><CIcon icon="cil-x" class="me-2" />Cancelar</CButton
        >
...
export default {
  ...
  methods: {
    ...
    async getListaEquipo() {
      // Funcion que crea una lista de Equipos Biometricos
    ...
    addEquipo() {
      // funcion para el registro de un Equipo
      this.$swal
        .fire({
          ...
          if (result.isConfirmed) {
            this.sccService.addEquipo(this.equipo).then((response) => {
              if (response.status == 200) {
                this.$swal
                  .fire("Equipo Registrado Corectamente", "", "success")
                  .then((result) => {
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.sccService.updateEquipo(this.equipoUpdate).then((response) => {
              if (response.status == 200) {
                this.$swal
                  .fire("Equipo Actualizado Corectamente", "", "success")
                  .then((result) => {
                    if (result) location.reload();
                  });
              } else {
