        ...
        <CCol :lg="12">
            <CCard>
                <CCardHeader class="headercolor">
                    <CRow>
                        <CCol :lg="6"><label class="d-none d-md-flex me-auto">Modulos Del Empleado</label> </CCol>
                        <CCol :lg="6" class="text-end">
                            <CDropdown variant="btn-group">
                                <CDropdownToggle  color="success" class="font" size="sm"><CIcon icon="cil-menu" class="me-2"/>Opciones</CDropdownToggle>
                                <CDropdownMenu>
                                    <CDropdownItem><CButton @click="clickModalModulo(true)" size="sm">Agregar Modulo</CButton></CDropdownItem>
                                    <CDropdownDivider/>
                                    <CDropdownItem><CButton @click="clickModalContrato(true)" size="sm">Agregar Contrato</CButton></CDropdownItem>
                                </CDropdownMenu>
                            </CDropdown>
                        </CCol>
                    </CRow>
                ...
                <CCardBody>
                    ...
                    <CCol :xs="12">
                        <CNav variant="tabs" class="nav nav-tabs">
                            <CNavItem><CNavLink :active="tab == 1" @click="clicktab(1)"><CButton size="sm"><CIcon icon="cil-command"></CIcon><label class="d-none d-md-flex me-auto">Modulos</label></CButton></CNavLink></CNavItem>
                            <CNavItem><CNavLink :active="tab == 2" @click="clicktab(2)"><CButton size="sm"><CIcon icon="cil-handshake"></CIcon><label class="d-none d-md-flex me-auto">Contratos</label></CButton></CNavLink></CNavItem>
                            <CNavItem><CNavLink :active="tab == 3" @click="clicktab(3)"><CButton size="sm"><CIcon icon="cil-menu"></CIcon><label class="d-none d-md-flex me-auto">Menu</label></CButton></CNavLink></CNavItem>
                        </CNav>
                        ...
                        <CTabContent>
                            ...
                            <CTabPane :visible="tab == 2">
                                ...
                                <CCol :xs="12" class="table-responsive">
                                    <table class="table">
                                        <thead>
                                            <tr>
                                                <th>#</th><th>N° Contrato</th><th>Servicio</th><th>Fechas</th><th>Detalle</th><th></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr v-for="contrato in contratos" :key="contrato.id">
                                                ...
                                                <td><CButton color="warning" class="font" @click="getContrato(contrato.id)"><CIcon icon="cil-pencil"></CIcon></CButton></td>
                                            </tr>
                            ...
                            <CTabPane :visible="tab == 3">
                                <br>
                                <CCol :xs="12" class="table-responsive">
                                    <h3>ID: {{ menu.id }}</h3><h3>Titulo: {{ menu.titulo }}</h3> 
                                    <table class="table">
                                        <thead>
                                            <tr>
                                                <th>#</th><th>Titulo</th><th>Ruta</th><th>Estado</th><th></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr v-for="m in menu.menuObj" :key="m.id">
                                                ...
                                                <td>{{ m.ruta }}</td>
                                                <td>
                                                    <CBadge v-if="m.estado == 1" color="success">Activo</CBadge>
                                                    <CBadge v-else color="danger">Inactivo</CBadge>
                                                </td>
                                                <td>
                                                    <CButton v-if="m.estado == 1" color="danger" class="font" @click="updateMenuUsuario(m.id,m.idmenu,0)" size="sm"><CIcon icon="cil-trash"></CIcon></CButton>
                                                    <CButton v-else color="success" class="font" @click="updateMenuUsuario(m.id,m.idmenu,1)" size="sm"><CIcon icon="cil-check-alt"></CIcon></CButton>
                                                </td>
                                            </tr>
                                        </tbody>
                                        <tfoot>
...
export default {
    ...
    methods: {
        ...
        async getEmpleado() {//Funcion para listar Los Datos del empleado como tambien sus contratos
        ...
        async addEmpleado() {// Funcion para registrar al Empleado con el tipo de empleado
                ...
                if (result.isConfirmed) {
                    this.usuarioService.addPerfil(this.emp.cif).then(response =>{
                        if(response.status == 200){
                            this.empleadoService.addEmpleado(this.emp.cif, this.id_tipoEmpleado).then(response => {
                                if (response.status == 200) {
        ...
        async addContrato() { // Funcion para Agregar Un contrato Al empelado 
            this.usuarioService.headersUsuario(this.usuario.token);
        ...
        async updateMenuUsuario(id,idmenu,estado){
            ...
            this.menuService.headersUsuario(this.usuario.token);
            await this.$swal.fire({
                title: ms+this.egovf.nombre +' ' +this.egovf.paterno+' '+this.egovf.materno+'?',
                showDenyButton: true,
                icon:'info',
                confirmButtonText: 'Aceptar',
                denyButtonText: 'Cancelar',
                ...
                if (result.isConfirmed) {
                    this.menuService.updateMenuUsuario(id,this.egovf.cif,idmenu,estado).then(response =>{
                        if(response.status == 200){
                            this.$swal.fire('Se Actualizaron los datos Corectamente', '', 'success').then((res) => {
                                if (res)
                                    location.reload();
                            });
                        }
