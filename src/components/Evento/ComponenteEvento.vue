  ...
  <CRow>
    <CCol :lg="6">
      ...
      <CWidgetStatsB class="mb-3" :progress="{ color: eventoDetalle.color, value: 100 }">
        ...
        <template #title><strong>{{ formatearFecha(eventoDetalle.fechaInicio) }}</strong> De <span
            class="small text-medium-emphasis">{{ eventoDetalle.horaInicio }}</span> a <span
            class="small text-medium-emphasis">{{ eventoDetalle.horaFin }}</span></template>
...
export default {
  ...
  methods: {
    formatearFecha(fechaStr) {
      if (typeof fechaStr !== 'string' || isNaN(fechaStr)) {
